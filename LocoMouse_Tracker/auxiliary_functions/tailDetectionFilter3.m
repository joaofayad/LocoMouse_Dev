function [Tail,T_mask,tail_length] = tailDetectionFilter3(Ii, Mi, mirror_line, w, rho, N, tail_length)
...
IB = conv2(double(I_cell{1}),w{1},'same')-rho{1};
IT = conv2(double(I_cell{2}),w{2},'same')-rho{2};

IB(~Mi{1}) = 0;
IT(~Mi{2}) = 0;

BB = imdilate(IB > 0,strel('square',5));
TT = imdilate(IT > 0,strel('square',5));
...
%     T_mask{2} = true(size(I_cell{2})); T_mask{2}= blackOutImageRegions(T_mask{2},ind_top',box_size{2});
    ...
    for i_views = 1:2
        for i_tail = 1:Ntail
            x = sum(BW{i_views}(:,steps(i_tail):steps(i_tail+1)),1);
            y = sum(BW{i_views}(:,steps(i_tail):steps(i_tail+1)),2);
